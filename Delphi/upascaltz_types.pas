...
type
  TTZDateTime=record
    ...
    class operator NotEqual(ADate, BDate: TTZDateTime): boolean;
    class operator Equal(ADate, BDate: TTZDateTime): boolean;
    class operator GreaterThan(ADate, BDate: TTZDateTime): boolean;
    class operator LessThan(ADate, BDate: TTZDateTime): boolean;
    class operator GreaterThanOrEqual(ADate, BDate: TTZDateTime): boolean;
    class operator LessThanOrEqual(ADate, BDate: TTZDateTime): boolean;
  end;
  ...
  public
    ...
    TimeZoneLetters: string;
  public
    function GetBeginDate(const AYear: Integer): TTZDateTime;
    function ToString: String; reintroduce; overload;
    function ToString(const Delimeter: String): String; reintroduce; overload;
  end;

  TTZRuleList = TObjectList<TTZRule>;
  ...
  TTZRuleGroupList = TObjectList<TTZRuleGroup>;
  ...
  TTZDateList = TObjectList<TTZDateListItem>;
  ...
  TTZZoneList = TObjectList<TTZZone>;
  ...
  TTZZoneGroupList = TObjectList<TTZZoneGroup>;
  ...
  TTZLinkList = TObjectList<TTZLink>;
...
uses
  uPascalTZ_Tools, System.Generics.Defaults;
...
begin
  ...
  begin
    ...
    if (Self.AtHourTimeForm <> TZ_TIME_FORM_DEFAULT) then
      AtTimeStr := AtTimeStr + string(TimeFormToChar(Self.AtHourTimeForm));
  end;
...
begin
//  Self.Sort(TDelegatedComparer<TTZZone>.Create(CompareZonesByValidUntil));
end;
...
begin
//  Self.Sort(TDelegatedComparer<TTZDateListItem>.Create(CompareDateListItems));
end;
...
class operator TTZDateTime.NotEqual(ADate, BDate: TTZDateTime): boolean;
begin
  Result := CompareDates(ADate, BDate) <> 0;
end;

class operator TTZDateTime.Equal(ADate, BDate: TTZDateTime): boolean;
begin
  Result := CompareDates(ADate, BDate) = 0;
end;

class operator TTZDateTime.GreaterThan(ADate, BDate: TTZDateTime): boolean;
begin
  Result := CompareDates(ADate, BDate) > 0;
end;

class operator TTZDateTime.LessThan(ADate, BDate: TTZDateTime): boolean;
begin
  Result:=CompareDates(ADate, BDate) < 0;
end;

class operator TTZDateTime.GreaterThanOrEqual(ADate, BDate: TTZDateTime): boolean;
begin
  Result := CompareDates(ADate, BDate) >= 0;
end;

class operator TTZDateTime.LessThanOrEqual(ADate, BDate: TTZDateTime): boolean;
begin
  Result:=CompareDates(ADate, BDate) <= 0;
end;
