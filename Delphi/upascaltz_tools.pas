...
function ConvertToTimeForm(const SourceSecondsInDay, StandardTimeOffset, SaveTimeOffset: Integer;
  const SourceTimeForm, TargetTimeForm: TTZTimeForm): Integer; overload;
function ConvertToTimeForm(const SourceDateTime: TTZDateTime; const StandardTimeOffset, SaveTimeOffset: Integer;
  const SourceTimeForm, TargetTimeForm: TTZTimeForm): TTZDateTime; overload;
...
begin
  ...
  Result := AsciiString(StringReplace(string(Result), '%s', string(ARuleLetters), [rfReplaceAll, rfIgnoreCase]));

  // When timezonename is XXX/YYY, XXX is no daylight and YYY is daylight saving.
  ZoneNameCut := Pos('/', string(Result));
...
begin
  if ADayName='Sun' then Result:=eTZSunday
    ...
    Raise TTZException.Create('Unknown day name: ' + string(ADayName));
end;
...
begin
  ...
  try
    Result.Year:=StrToInt(string(TmpWord));
  ...
  TmpWord := AIterator.GetNextWord;
  if TmpWord = '' then Exit;
  Result.Day := StrToIntDef(string(TmpWord), 0);
...
begin
  ...
  j:=StrToIntDef(string(ADayString),-1);
  if j<>-1 then
  begin
    ADate.Day:=j;
    Exit;
...
begin
  ...
  try
    ...
    case TwoColons of
      // Format is "h"
      0:  begin
            Hours := StrToInt(string(TmpTime));
          end;
      // Format is "hh:mm"
      1:  begin
            TimeIterator:=TTZLineIterate.Create(TmpTime,':');
            try
              Hours:=StrToInt(string(TimeIterator.GetNextWord));
              Minutes:=StrToInt(string(TimeIterator.GetNextWord));
            finally
              TimeIterator.Free;
            end;
          end;
      ...
      2:  begin
            ...
            try
              Hours:=StrToInt(string(TimeIterator.GetNextWord));
              Minutes:=StrToInt(string(TimeIterator.GetNextWord));
              Seconds:=StrToInt(string(TimeIterator.GetNextWord));
            finally
              TimeIterator.Free;
            end;
          end;
